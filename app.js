...
function getVisibleItems() { if(state.viewMode==="favorites") return getOneNextPerSeries(getFavoriteItems()); if(state.viewMode==="today") return sortByDate(state.releases.filter(item => isToday(item.releaseDate))); return getOneNextPerSeries(state.releases); }
